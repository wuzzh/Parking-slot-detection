...
void CPMDataTransformer<Dtype>::ReadMetaData(MetaData& meta, const string& data, size_t offset3, size_t offset1) { //very specific to genLMDB.py
  ...
  //LOG(INFO) << "dataname: " << meta.people_index
...
void CPMDataTransformer<Dtype>::TransformJoints(Joints& j) {
  ...
  // arrow key point
    ...
    iPoint_num = 3;
    int ARROW_to_ours_1[3] = {0,1,2};
    int ARROW_to_ours_2[3] = {0,1,2};
    jo.joints.resize(np);
    jo.isVisible.resize(np);

    for(int i=0;i<iPoint_num;i++){
      jo.joints[i] = (j.joints[ARROW_to_ours_1[i]] + j.joints[ARROW_to_ours_2[i]]) * 0.5;
      if(j.isVisible[ARROW_to_ours_1[i]]==2 || j.isVisible[ARROW_to_ours_2[i]]==2){
        jo.isVisible[i] = 2;
      }
      else {
        jo.isVisible[i] = j.isVisible[ARROW_to_ours_1[i]] && j.isVisible[ARROW_to_ours_2[i]];
      }
    }
  }
//------------------------------------------------parking key point-----------------------------------------------
// parking key point 
    ...
    iPoint_num = 6;
    int parking_key_point_1[6] = {0,1,2,3,4,5};
    int parking_key_point_2[6] = {0,1,2,3,4,5};
    jo.joints.resize(np);
    jo.isVisible.resize(np);

    for(int i=0;i<iPoint_num;i++){
      jo.joints[i] = (j.joints[parking_key_point_1[i]] + j.joints[parking_key_point_2[i]]) * 0.5;
      if(j.isVisible[parking_key_point_1[i]]==2 || j.isVisible[parking_key_point_2[i]]==2){
        jo.isVisible[i] = 2;
      }
      else {
        jo.isVisible[i] = j.isVisible[parking_key_point_1[i]] && j.isVisible[parking_key_point_2[i]];
      }
    }
 }
//------------------------------------------------guidepost key point-----------------------------------------------
//  guidepost key point
    ...
    iPoint_num = 4;
    int guidepost_to_ours_1[4] = {0,1,2,3};
    int guidepost_to_ours_2[4] = {0,1,2,3};
    jo.joints.resize(np);
    jo.isVisible.resize(np);

    for(int i=0;i<iPoint_num;i++){
      jo.joints[i] = (j.joints[guidepost_to_ours_1[i]] + j.joints[guidepost_to_ours_2[i]]) * 0.5;
      if(j.isVisible[guidepost_to_ours_1[i]]==2 || j.isVisible[guidepost_to_ours_2[i]]==2){
        jo.isVisible[i] = 2;
      }
      else {
        jo.isVisible[i] = j.isVisible[guidepost_to_ours_1[i]] && j.isVisible[guidepost_to_ours_2[i]];
      }
    }
  }
...
//  vechile key point
    ...
    iPoint_num = 8;
    int vechile_to_ours_1[8] = {0,1,2,3,4,5,6,7};
    int vechile_to_ours_2[8] = {0,1,2,3,4,5,6,7};
    jo.joints.resize(np);
    jo.isVisible.resize(np);

    for(int i=0;i<iPoint_num;i++){
      jo.joints[i] = (j.joints[vechile_to_ours_1[i]] + j.joints[vechile_to_ours_2[i]]) * 0.5;
      if(j.isVisible[vechile_to_ours_1[i]]==2 || j.isVisible[vechile_to_ours_2[i]]==2){
        jo.isVisible[i] = 2;
      }
      else {
        jo.isVisible[i] = j.isVisible[vechile_to_ours_1[i]] && j.isVisible[vechile_to_ours_2[i]];
      }
    }
  }
 //------------------------------------------------vechile key point-----------------------------------------------
...
Size CPMDataTransformer<Dtype>::augmentation_croppad(Mat& img_src, Mat& img_dst, Mat& mask_miss, Mat& mask_miss_aug, Mat& mask_all, Mat& mask_all_aug, MetaData& meta, int mode) {
  ...
  */
  ...
  float x_offset = -crop_x/2;
  float y_offset = -crop_y/2;
...
*/
      entry[g_y*grid_x + g_x] += exp(-exponent) * thresh;
      if(entry[g_y*grid_x + g_x] > thresh) 
        entry[g_y*grid_x + g_x] = thresh;
    }
  }
}
...
void CPMDataTransformer<Dtype>::generateLabelMap(Dtype* transformed_label, Mat& img_aug, MetaData meta) {
  ...
  int iPoint_num = 0;
  int iConnection_num = 0;
  int num_parts = 0; //connection_num * 2 + iPoint_num
  int iConn_bak = 0; // connection_num * 2 + 1
  int iChannel_num = 0; // connection_num * 2 + iPoint_num + 1(background)
...
//---------------------------------------------------arrow key point--------------------------------------------------------------------------
  ...
  iPoint_num = 3;
  iConnection_num = 2;
  num_parts = 7; //connection_num * 2 + iPoint_num
  iConn_bak = 5; // connection_num * 2 + 1
  iChannel_num = 8; // connection_num * 2 + iPoint_num + 1(background)
  
  if (np == num_parts){
    for (int i = 0; i < iPoint_num; i++){
      Point2f center = meta.joint_self.joints[i];
      if(meta.joint_self.isVisible[i] <= 1){
        putGaussianMaps(transformed_label + (i+np+iConn_bak)*channelOffset, center, param_.stride(), 
    ...
    for(int i=0;i<iConnection_num;i++){
    ...
    for (int g_y = 0; g_y < grid_y; g_y++){
      for (int g_x = 0; g_x < grid_x; g_x++){
        ...
        for (int i = np+iConn_bak; i < np+iChannel_num; i++){
...
//---------------------------------------------------parking key point--------------------------------------------------------------------------
  ...
  iPoint_num = 6;
  iConnection_num = 8;
  num_parts = 22; //connection_num * 2 + iPoint_num
  iConn_bak = 17; // connection_num * 2 + 1
  iChannel_num = 23; // connection_num * 2 + iPoint_num + 1(background)
  ...
  if (np == num_parts){
    ...
    int mid_1[8] = {1, 2, 3, 4, 5, 5, 6, 6};  
    int mid_2[8] = {2, 3, 4, 1, 6, 4, 3, 5}; 
    int thre = 1;
  ...
  iPoint_num = 4;
  iConnection_num = 4;
  num_parts = 12; //connection_num * 2 + iPoint_num
  iPoint_num = 4;
  iConn_bak = 9; // connection_num * 2 + 1
  iChannel_num = 13; // connection_num * 2 + iPoint_num + 1(background)
...
//--------------------------------------------------- vechile  key point--------------------------------------------------------------------------
  ...
  iPoint_num = 8;
  iConnection_num = 12;
  num_parts = 32; //connection_num * 2 + iPoint_num
  iConn_bak = 25; // connection_num * 2 + 1
  iChannel_num = 33; // connection_num * 2 + iPoint_num + 1(background)
  ...
  if (np == num_parts){
    ...
    int mid_1[12] = {1, 1, 1, 2, 2, 3, 3, 4, 5, 5, 6, 7};  
    int mid_2[12] = {2, 4, 5, 3, 6, 4, 7, 8, 6, 8, 7, 8}; 
    int thre = 1;
